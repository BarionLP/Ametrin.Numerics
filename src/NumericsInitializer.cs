...
public static class NumericsInitializer
{
    ...
    public static void KaimingUniform(this Matrix w, Weight gain, Random random, FanMode mode = FanMode.FanIn)
    {
        var fan = Fan(w, mode);
        var bound = gain * Weight.Sqrt((Weight)6 / fan);
        w.Uniform(-bound, bound, random);
    }
    public static void KaimingNormal(this Matrix w, Weight gain, Random random, FanMode mode = FanMode.FanIn)
    {
        var fan = Fan(w, mode);
    ...
    public static void KaimingUniform(this Tensor t, int fanInAxis, Weight gain, Random random, FanMode mode = FanMode.FanIn)
    {
        ...
        var bound = gain * Weight.Sqrt(6 / fan);
        t.Uniform(-bound, bound, random);
    }
    public static void KaimingNormal(this Tensor t, int fanInAxis, Weight gain, Random random, FanMode mode = FanMode.FanIn)
    {
        var (fi, fo) = Fans(t, fanInAxis, mode is FanMode.FanOut ? fanInAxis : fanInAxis);
        int fan = mode switch
        {
        ...
        var std = gain * Weight.Sqrt(2 / fan);
        t.Normal(0, std, random);
    }
